...
## Phase 2 / BattleSimulation — IMMUTABLE setup row for a single
## battle participant.
##
...
##
## After construction, fields are NOT intended to be mutated by
## the simulation. The simulation copies them into its own
## component tables.
##
...
func _init(
	...
	source_run_unit_id = p_source_run_unit_id
	definition_id = p_definition_id
	team = p_team
	cell = p_cell
	starting_hp = p_starting_hp
	max_hp = p_max_hp
	attack_base = p_attack_base
	defense_base = p_defense_base
